import { applySafePlaybookIgnoreRecommendations, suggestPlaybookIgnore } from '@zachariahredfield/playbook-engine';
...
const printSuggestText = (result) => {
    ...
    for (const entry of result.recommendations) {
        ...
        console.log(`- #${entry.rank} ${entry.path} [${entry.safety_level}] [${coverage}] [${applyState}] ${entry.expected_scan_impact.impact_level}`);
        console.log(`  rationale: ${entry.rationale}`);
    }
};
const printApplyText = (result) => {
...
export const runIgnore = async (cwd, commandArgs, options) => {
