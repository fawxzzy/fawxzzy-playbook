...
const printArchitectureHelp = () => {
    console.log(`Usage: playbook architecture verify [--json]`);
};
export const runArchitecture = async (cwd, args, options) => {
    ...
    if (!options.quiet) {
        ...
        console.log(result.errors.some((error) => error.startsWith('Duplicate artifact ownership')) ? '✗ duplicate ownership detected' : '✓ no duplicate ownership');
        console.log(result.errors.some((error) => error.startsWith('Unknown command mapping')) ? '✗ command mapping invalid' : '✓ command mapping valid');
