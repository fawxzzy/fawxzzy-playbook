...
The graph feeds a compressed module-runtime layer at `.playbook/context/modules/*.json` generated by `playbook index`. These digests are layer-relative knowledge units (identity, dependencies/dependents, docs/tests/risk summaries, graph-neighborhood kinds) reused by existing read surfaces (`query impact`, `explain <module>`) to reduce repeated broad inference.
