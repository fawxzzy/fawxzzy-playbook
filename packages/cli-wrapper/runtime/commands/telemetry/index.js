...
const LEARNING_CLUSTERS_PATH = ['.playbook', 'learning-clusters.json'];
const REPO_GRAPH_PATH = ['.playbook', 'repo-graph.json'];
const HIGHER_ORDER_SYNTHESIS_PATH = ['.playbook', 'higher-order-synthesis.json'];
...
const deriveLearningClustersReadModel = (cwd) => {
    const artifact = readJsonArtifact(cwd, LEARNING_CLUSTERS_PATH);
    const repoGraph = tryReadJsonArtifact(cwd, REPO_GRAPH_PATH);
    const higherOrderSynthesis = tryReadJsonArtifact(cwd, HIGHER_ORDER_SYNTHESIS_PATH);
    ...
    const moduleNameById = new Map(moduleNodes.map((node) => [node.id, node.name ?? String(node.id).replace(/^module:/, '')]));
    const dependencyEdges = (repoGraph?.edges ?? []).filter((edge) => edge.kind === 'depends_on' &&
        typeof edge.from === 'string' &&
        typeof edge.to === 'string' &&
        moduleIds.has(edge.from) &&
        moduleIds.has(edge.to));
    const degreeByModule = new Map();
    for (const moduleId of moduleIds) {
        degreeByModule.set(moduleId, 0);
    }
    for (const edge of dependencyEdges) {
        degreeByModule.set(edge.from, (degreeByModule.get(edge.from) ?? 0) + 1);
        degreeByModule.set(edge.to, (degreeByModule.get(edge.to) ?? 0) + 1);
    }
    ...
    const concentrationLabel = affectedModuleCount === 0 ? 'none' : concentrationIndex >= 0.6 ? 'concentrated' : 'distributed';
    const topAffectedModule = affectedModules[0];
    const graphReviewAction = affectedModuleCount === 0
...
const renderTextLearningClusters = (readModel) => {
    ...
    console.log(`Graph-informed affected modules: ${readModel.graph_informed.affected_modules.length > 0 ? readModel.graph_informed.affected_modules.join(', ') : 'none'}`);
    console.log(`Structural spread: ${readModel.graph_informed.structural_spread.concentration_label} (affected=${readModel.graph_informed.structural_spread.affected_module_count}/${readModel.graph_informed.structural_spread.module_count}, concentration=${readModel.graph_informed.structural_spread.concentration_index})`);
