...
const PLAN_ARTIFACT_RELATIVE_PATH = '.playbook/plan.json';
const renderTextPlan = (tasks) => {
    console.log('Plan');
    console.log('────────');
    console.log('');
...
export const runPlan = async (cwd, options) => {
    ...
    engine.appendExecutionStep(cwd, runId, {
        ...
        evidence: [
            ...(options.outFile ? [{ id: 'evidence-plan-artifact', kind: 'artifact', ref: options.outFile }] : [])
        ]
    });
    ...
    engine.attachSessionRunState(cwd, {
        ...
        artifacts: [
            ...
            ...(options.outFile ? [{ artifact: options.outFile, kind: 'plan' }] : [])
        ]
    });
