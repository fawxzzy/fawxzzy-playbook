import { readCrossRepoPatternsArtifact, readPortabilityOutcomesArtifact } from '@zachariahredfield/playbook-engine';
...
const ensureArtifactExists = (cwd, relativePath) => {
...
const sortTransferPlans = (records) => [...records].sort((left, right) => right.portability_confidence - left.portability_confidence ||
    left.pattern.localeCompare(right.pattern) ||
    left.target_repo.localeCompare(right.target_repo));
const readTransferPlansArtifact = (cwd) => {
    ...
    return {
        ...
        plans: sortTransferPlans(transferPlans.map((entry) => {
            const typed = entry;
            return {
...
const sortTransferReadiness = (records) => [...records].sort((left, right) => right.readiness_score - left.readiness_score ||
    left.pattern.localeCompare(right.pattern) ||
    left.target_repo.localeCompare(right.target_repo));
const readTransferReadinessArtifact = (cwd) => {
    ...
    return {
        ...
        assessments: sortTransferReadiness(readiness.map((entry) => {
            const typed = entry;
            const subsystemPresence = (typed.subsystem_presence ?? {});
            const artifactAvailability = (typed.artifact_availability ?? {});
            const validationCoverage = (typed.validation_coverage ?? {});
            const governanceAlignment = (typed.governance_alignment ?? {});
            return {
                pattern: String(typed.pattern_id ?? typed.pattern ?? 'unknown'),
...
const inferCompatibleSubsystems = (patternId) => {
...
const resolveSourceRepo = (repositories, patternId) => {
...
const derivePortabilityRecords = (artifact) => artifact.aggregates.map((entry) => ({
...
const deriveRecommendations = (artifact) => {
    ...
    for (const aggregate of artifact.aggregates) {
        ...
        const reposWithPattern = new Set(artifact.repositories
    ...
    return recommendations.sort((left, right) => right.initial_portability_score - left.initial_portability_score ||
        left.pattern.localeCompare(right.pattern) ||
        left.target_repo.localeCompare(right.target_repo));
};
const classifyOutcome = (portabilityScore) => {
...
const classifyAdoption = (portabilityScore) => {
...
const deriveOutcomesFromArtifact = (artifact, recommendations) => {
...
const deriveOutcomes = (cwd, recommendations) => {
    const recorded = readPortabilityOutcomesArtifact(cwd);
...
const deriveRecalibration = (artifact, outcomes) => {
    ...
    return outcomes.map((outcome) => {
        ...
        const outcomeMultiplier = outcome.observed_outcome === 'successful' ? 1 : outcome.observed_outcome === 'inconclusive' ? 0.92 : 0.8;
...
export const parsePortabilityView = (args) => {
...
export const runKnowledgePortability = (cwd, view = 'overview') => {
