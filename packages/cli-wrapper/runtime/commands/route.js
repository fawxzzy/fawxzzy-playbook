...
import { buildExecutionPlan, routeTask, compileCodexPrompt, recordRouteDecision, safeRecordRepositoryEvent, readStoriesArtifact, findStoryById, buildStoryRouteTask, toStoryPlanningReference, deriveStoryTransitionPreview, linkStoryToPlan, validateStoriesArtifact, STORIES_RELATIVE_PATH, buildStoryPatternContext, } from "@zachariahredfield/playbook-engine";
...
import { buildRouteInterpretation, } from "../lib/interpretation.js";
...
const tryReadJsonArtifact = (cwd, artifactPath) => {
...
const normalizeStoryForTransition = (story) => ({
    ...story,
...
const normalizeStoriesArtifactForTransition = (artifact) => ({
    ...artifact,
    stories: artifact.stories.map((story) => normalizeStoryForTransition(story)),
});
const promoteStoryTransition = (cwd, current, storyId) => {
    const normalizedCurrent = normalizeStoriesArtifactForTransition(current);
    const transition = deriveStoryTransitionPreview(normalizedCurrent, storyId, "planned");
    if (!transition) {
        return null;
    }
    const generatedAt = new Date().toISOString();
    const nextArtifact = linkStoryToPlan(normalizedCurrent, storyId, EXECUTION_PLAN_PATH, generatedAt);
...
const toOutput = (task, decision, executionPlan, promotion, codexPrompt, storyTransition, patternContext, storyContext) => ({
    schemaVersion: "1.0",
    command: "route",
    task,
...
const printText = (payload, options) => {
    console.log("State");
    ...
    console.log(payload.interpretation.progressive_disclosure.default_view.next_step
        .command ||
    ...
    console.log(`Pattern context matches: ${payload.pattern_context.patterns.length}`);
    console.log("");
...
export const runRoute = async (cwd, commandArgs, options) => {
    if (options.help) {
        printCommandHelp({
    ...
    const storyTransition = linkedStory && storiesArtifact
        ? promoteStoryTransition(cwd, storiesArtifact, linkedStory.id)
        : null;
    const output = toOutput(task, decision, executionPlan, promotion, codexPrompt, storyTransition, patternContext, linkedStory
        ? {
            id: linkedStory.id,
