...
- `playbook init`
- `playbook analyze` (also writes `.playbook/repo-index.json` with framework/language, module topology, docs, rules, and architecture hints for machine-readable indexing)
- `playbook verify`
- `playbook doctor`
- `playbook diagram`
...
Example `repo-index.json` shape generated by `playbook analyze`:

```json
{
  "framework": "node",
  "language": "typescript",
  "modules": ["src/features/users", "src/features/workouts"],
  "shared_modules": ["src/shared"],
