...
const buildStoryPatternContext = vi.fn(() => ({ patterns: [] }));

vi.mock("@zachariahredfield/playbook-engine", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@zachariahredfield/playbook-engine")>();
  return {
    ...actual,
    routeTask,
    buildExecutionPlan,
...
describe("runRoute", () => {
  it("returns deterministic failure for missing task argument", { timeout: 15000 }, async () => {
    ...
    const exitCode = await runRoute("/repo", [], {
      format: "json",
      quiet: false,
      codexPrompt: false,
    });
  ...
  it("prints help without writing artifacts", async () => {
    ...
    const exitCode = await runRoute(repo, ["--help"], {
      format: "text",
      quiet: false,
      codexPrompt: false,
      help: true,
    ...
    expect(
      fs.existsSync(path.join(repo, ".playbook", "execution-plan.json")),
    ).toBe(false);
  ...
  it("emits deterministic json route output with execution plan proposal", async () => {
    ...
    buildStoryPatternContext.mockReturnValue({
      patterns: [
        {
          pattern_id: "pattern.docs",
          why_matched: "explicit_pattern_reference",
          provenance_refs: ["pattern.docs"],
          freshness: {
            status: "promoted",
    ...
    buildStoryPatternContext.mockReturnValue({ patterns: [] });

    const exitCode = await runRoute(repo, ["update", "command", "docs"], {
      format: "json",
      quiet: false,
      codexPrompt: true,
    });
    ...
    expect(
      payload.interpretation.progressive_disclosure.default_view.next_step
        .command,
    ).toBe("pnpm playbook docs audit --json");
    expect(payload.pattern_context.patterns).toEqual([]);
    ...
    const persisted = JSON.parse(
      fs.readFileSync(
        path.join(repo, ".playbook", "execution-plan.json"),
        "utf8",
      ),
    );
  ...
  it("routes from a story id with legacy/partial story shape and preserves story linkage in plan metadata", async () => {
    ...
    findStoryById.mockImplementation((_artifact, id) =>
      id === "story-1" ? story : null,
    );
    buildStoryRouteTask.mockReturnValue(
      "update command docs: Update command docs",
    );
    ...
    const exitCode = await runRoute(repo, ["--story", "story-1"], {
      format: "json",
      quiet: false,
      codexPrompt: false,
    });
    ...
    expect(payload.executionPlan.story_reference.story_reference).toBe(
      "story:story-1",
    );
    ...
    expect(buildExecutionPlan).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        patternContext: expect.objectContaining({
