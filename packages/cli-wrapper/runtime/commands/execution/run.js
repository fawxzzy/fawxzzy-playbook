...
import { buildFleetAdoptionWorkQueue, buildFleetCodexExecutionPlan, evaluateRuntimeCapabilityAuthorization, readInteropRuntime, WORKER_LAUNCH_PLAN_RELATIVE_PATH } from '@zachariahredfield/playbook-engine';
...
const parseWorkerAdapterResult = (raw, expectedPromptId) => {
...
const invokeWorkerAdapter = (command, payload, cwd) => {
...
const buildAdapterEnvelope = (prompt) => ({
    schemaVersion: '1.0',
...
const normalizeLaunchArtifactInput = (value) => {
    if (value && typeof value === 'object' && value.kind === 'worker-launch-plan' && Array.isArray(value.lanes)) {
        return value;
    }
    return undefined;
};
const validateLaunchPlan = (cwd, worksetPlan, launchPlan) => {
    if (!launchPlan) {
        return {
            ok: false,
    ...
    const blockedLanes = launchPlan.lanes
        .filter((lane) => !lane.launchEligible)
        .map((lane) => ({ lane_id: lane.lane_id, blockers: [...lane.blockers].sort((a, b) => a.localeCompare(b)) }))
    ...
    const eligibleLanes = launchPlan.lanes
        .map((lane) => ({ lane_id: lane.lane_id, launchEligible: lane.launchEligible, blockers: [...lane.blockers] }))
        .filter((lane) => lane.launchEligible)
        .sort((left, right) => left.lane_id.localeCompare(right.lane_id));
    ...
    if (!runtimeCapabilityAuthorization.ok) {
        const blockerSummary = runtimeCapabilityAuthorization.blockers
            .map((blocker) => `${blocker.lane_id}: ${blocker.blocker_code}; capabilities=${blocker.blocked_capability_ids.join(',') || 'none'}`)
            .join(' | ');
        return {
            ok: false,
...
const runWorkerBridge = async (cwd, options, tracker) => {
...
export const runExecution = async (cwd, options) => {
    ...
    try {
        ...
        const rawLaunchPlan = readJsonArtifact(cwd, WORKER_LAUNCH_PLAN_RELATIVE_PATH);
