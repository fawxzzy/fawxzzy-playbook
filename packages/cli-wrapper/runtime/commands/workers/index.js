...
import { assignWorkersToLanes, buildAssignedPrompt, buildChangeScopeBundleFromWorkerLaunchPlan, buildWorkerLaunchPlan, deriveLaneState, mergeWorkerResult, readWorkerResultsArtifact, recordLaneOutcome, recordWorkerAssignment, safeRecordRepositoryEvent, validateWorkerResultInput, validateWorkerSubmitAgainstScope, writeChangeScopeArtifact, writeWorkerLaunchPlanArtifact, writeWorkerResultsArtifact, WORKER_LAUNCH_PLAN_RELATIVE_PATH, WORKER_RESULTS_RELATIVE_PATH } from '@zachariahredfield/playbook-engine';
...
const writeJsonArtifact = (cwd, artifactPath, payload) => {
    const absolutePath = path.join(cwd, artifactPath);
    fs.mkdirSync(path.dirname(absolutePath), { recursive: true });
    fs.writeFileSync(absolutePath, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
};
const resolveLaneState = (cwd, worksetPlan) => deriveLaneState(worksetPlan, WORKSET_PLAN_PATH, { workerResults: readWorkerResultsArtifact(cwd) });
const writeAssignedPrompts = (cwd, assignments, worksetPlan) => {
    const lanePromptsById = new Map((worksetPlan?.lanes ?? []).map((lane) => [lane.lane_id, lane.codex_prompt]));
    const written = [];
...
const printAssignText = (assignments) => {
...
const parseWorkerResultInput = (cwd, from) => {
    const absolutePath = path.isAbsolute(from) ? from : path.join(cwd, from);
    return JSON.parse(fs.readFileSync(absolutePath, 'utf8'));
};
const runAssign = async (cwd, options, worksetPlan) => {
...
const runLaunchPlan = async (cwd, options, worksetPlan) => {
    const laneState = resolveLaneState(cwd, worksetPlan);
    writeJsonArtifact(cwd, LANE_STATE_PATH, laneState);
    const assignments = assignWorkersToLanes(laneState, worksetPlan);
    writeJsonArtifact(cwd, WORKER_ASSIGNMENTS_PATH, assignments);
...
const runSubmit = async (cwd, options, worksetPlan) => {
    if (!options.from) {
    ...
    writeJsonArtifact(cwd, LANE_STATE_PATH, laneState);
    const laneEntry = laneState.lanes.find((lane) => lane.lane_id === merged.result.lane_id);
    const affectedSurfaces = [
        ...merged.result.fragment_refs.map((ref) => ref.target_path),
        ...merged.result.proof_refs.map((ref) => ref.path),
        ...merged.result.artifact_refs.map((ref) => ref.path)
...
export const runWorkers = async (cwd, options) => {
    ...
    if (options.action === 'submit')
        return runSubmit(cwd, options, worksetPlan);
    if (options.action === 'launch-plan')
        return runLaunchPlan(cwd, options, worksetPlan);
