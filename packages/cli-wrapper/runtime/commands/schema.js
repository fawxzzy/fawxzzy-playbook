...
const renderTextSummary = (command) => {
    if (command) {
...
const withQueryAdditiveMemoryFields = (schema) => {
...
const withExplainMemoryKnowledgeFields = (schema) => {
    const oneOf = schema.oneOf;
    if (!Array.isArray(oneOf))
        return;
...
const withPlanAdvisoryFields = (schema) => {
...
const withAnalyzePrAdditions = (schema) => {
    const oneOf = schema.oneOf;
    if (!Array.isArray(oneOf) || oneOf.length < 2)
        return;
    const success = asRecord(oneOf[1]);
    const required = success.required;
    if (Array.isArray(required) && !required.includes('preventionGuidance'))
        required.push('preventionGuidance');
...
const withContractsSchemaRegistry = (schema) => {
    const required = schema.required;
    if (Array.isArray(required) && !required.includes('schemas'))
        required.push('schemas');
    const properties = asRecord(schema.properties ?? {});
    properties.schemas = {
        type: 'object',
        additionalProperties: false,
...
const applySchemaAdditions = (target, payload) => {
    const cloned = JSON.parse(JSON.stringify(payload));
    if (target) {
        if (target === 'query')
            withQueryAdditiveMemoryFields(cloned);
        if (target === 'explain')
            withExplainMemoryKnowledgeFields(cloned);
        if (target === 'plan')
            withPlanAdvisoryFields(cloned);
        if (target === 'analyze-pr')
            withAnalyzePrAdditions(cloned);
        if (target === 'contracts')
            withContractsSchemaRegistry(cloned);
        return cloned;
    }
    const all = cloned;
...
export const runSchema = async (_cwd, commandArgs, options) => {
