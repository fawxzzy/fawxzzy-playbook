...
import { analyzePullRequest, buildPrReviewLoopArtifact, evaluateImprovementPolicy, generateImprovementCandidates, writePrReviewLoopArtifact } from '@zachariahredfield/playbook-engine';
...
const REVIEW_PR_ARTIFACT_RELATIVE_PATH = '.playbook/pr-review.json';
const REVIEW_PR_LOOP_ARTIFACT_RELATIVE_PATH = '.playbook/pr-review-loop.json';
const deterministicStringify = (value) => `${JSON.stringify(value, null, 2)}\n`;
const writeReviewPrArtifact = (cwd, artifact) => {
...
const toFailureRuntime = (options) => ({
    format: options.format === 'json' ? 'json' : 'text',
    quiet: options.quiet
});
const validateReviewPrFormat = (format) => {
...
const toPolicyGroups = (evaluations) => ({
    safe: evaluations.filter((evaluation) => evaluation.decision === 'safe'),
    requires_review: evaluations.filter((evaluation) => evaluation.decision === 'requires_review'),
    blocked: evaluations.filter((evaluation) => evaluation.decision === 'blocked')
});
const createPolicyArtifactFromCandidates = (improvementArtifact, cwd) => {
...
const toReviewPrArtifact = (input) => {
    ...
    const scmSource = input.analysis.context.sources.find((source) => source.type === 'scm-context');
    return {
        schemaVersion: '1.0',
        kind: 'pr-review',
        findings,
...
const renderGithubComment = (artifact) => {
    const lines = [
        '## ✅ Playbook PR Review',
        '',
        '| Category | Count |',
...
export const runReviewPr = async (cwd, options) => {
