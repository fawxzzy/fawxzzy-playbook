...
const isReportOnlyCommand = () => command === 'learn' && commandArgs.find((arg) => !arg.startsWith('-')) === 'doctrine';
...
const resolveRuntimeRepoRoot = (fallbackRoot) => {
    if (command !== 'pilot') {
        return fallbackRoot;
    }
...
const run = async () => {
    ...
    const childCommands = command === 'pilot' ? ['context', 'index', 'query modules', 'verify', 'plan'] : [];
