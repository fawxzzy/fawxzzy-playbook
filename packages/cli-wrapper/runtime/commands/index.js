...
const parseOptionValues = (allArgs, name) => {
    const values = [];
...
const parseLearnDiffContext = (allArgs) => {
    if (allArgs.includes("--no-diff-context")) {
        return false;
    }
    return true;
};
const parseAnalyzePrFormat = (allArgs, globalFormat) => {
...
const parseReviewPrFormat = (allArgs, globalFormat) => {
    if (globalFormat === "json") {
        return "json";
    }
    const format = parseOptionValue(allArgs, "--format");
...
const parseVerifyFormat = (allArgs, globalFormat) => {
    if (globalFormat === "json") {
        return "json";
    }
    return parseOptionValue(allArgs, "--format") === "sarif" ? "sarif" : "text";
};
const parseOrchestrateArtifactFormat = (allArgs, globalFormat) => {
    if (globalFormat === "json") {
        return "json";
    }
    const format = parseOptionValue(allArgs, "--format");
...
const commandRunners = {
    ...
    verify: async ({ cwd, args, commandArgs, ci, explain, format, quiet }) => {
        ...
        return runVerify(cwd, {
            ...
            phase: parseOptionValue(commandArgs, "--phase"),
    ...
    improve: async ({ cwd, commandArgs, format, quiet }) => {
        const { runImprove, runImproveCommands, runImproveOpportunities, runImproveApplySafe, runImproveApprove, } = await import("./improve.js");
    ...
    learn: async ({ cwd, commandArgs, format, quiet }) => {
        ...
        if (subcommand !== "draft" && subcommand !== "doctrine") {
            const message = 'playbook learn: unsupported subcommand. Use "playbook learn draft" or "playbook learn doctrine".';
            if (format === "json") {
                console.log(JSON.stringify({ schemaVersion: "1.0", command: "learn", error: message }, null, 2));
            }
            else {
                console.error(message);
            }
...
export const commandRegistry = commandOrder.map((name) => {
...
export const hasRegisteredCommand = (commandName) => commandMap.has(commandName);
export const runRegisteredCommand = async (commandName, context) => {
