import { listRuntimeLogRecords, listRuntimeRuns, listRuntimeTasks, readRuntimeControlPlaneStatus, readRuntimeRun, runAgentPlanDryRun, writeControlPlaneState } from '@zachariahredfield/playbook-engine';
import { emitJsonOutput } from '../lib/jsonArtifact.js';
import { ExitCode } from '../lib/cliContract.js';
const printAgentHelp = () => {
...
const requireRunId = (args, command) => {
...
const resolvePositionalRunId = (args) => {
...
export const runAgent = async (cwd, args, options) => {
    ...
    try {
        ...
        if (subcommand === 'run') {
            ...
            if (options.format === 'json') {
                ...
                console.log(`Dry-run compiled ${payload.compiledTaskCount} tasks (ready=${payload.readyBlockedSummary.readyCount}, blocked=${payload.readyBlockedSummary.blockedCount}).`);
            }
            return ExitCode.Success;
        }
        if (subcommand === 'tasks') {
