import { applyAutoSafeImprovements, approveGovernanceImprovement, generateImprovementCandidates, writeImprovementCandidatesArtifact } from '@zachariahredfield/playbook-engine';
        ...
        extraSections: [
            {
                ...
                items: artifact.candidates.slice(0, 3).map((candidate) => `${candidate.candidate_id} [${candidate.gating_tier}] — ${candidate.suggested_action}`)
            },
            {
                ...
                items: [
                    topRouterRecommendation
                        ...
                        : '',
                    ...artifact.doctrine_promotions.transitions.slice(0, 2).map((transition) => `${transition.candidate_id}: ${transition.from_stage} -> ${transition.to_stage}`)
                ].filter(Boolean)
            }
        ]
    }));
};
const renderOpportunityText = (artifact) => {
    ...
    console.log(renderBriefOutput({
        ...
        extraSections: [{
                label: 'Ranked queue',
                items: items.slice(0, 3).map((entry, index) => `#${index + 1} ${entry.title} (${entry.heuristic_class}, priority ${entry.priority_score}, confidence ${entry.confidence})`)
            }]
    }));
};
...
export const runImproveOpportunities = async (cwd, options) => {
    if (options.help) {
        printImproveHelp();
...
export const runImproveCommands = async (cwd, options) => {
    if (options.help) {
        printImproveHelp();
        return ExitCode.Success;
    }
...
export const runImprove = async (cwd, options) => {
    if (options.help) {
        printImproveHelp();
        return ExitCode.Success;
    }
...
export const runImproveApplySafe = async (cwd, options) => {
    if (options.help) {
        printImproveHelp();
        return ExitCode.Success;
    }
...
export const runImproveApprove = async (cwd, proposalId, options) => {
    if (options.help) {
        printImproveHelp();
        return ExitCode.Success;
    }
