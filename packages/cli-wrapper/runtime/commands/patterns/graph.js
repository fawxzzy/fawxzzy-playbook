...
const PATTERNS_PATH = ['.playbook', 'memory', 'knowledge', 'patterns.json'];
const CONTRACT_PATTERN_GRAPH_PATH = ['.playbook', 'pattern-graph.json'];
export const readContractPatternGraph = (cwd) => {
...
export const readPatternKnowledgeGraph = (cwd) => {
    ...
    edges.sort((left, right) => left.relation.localeCompare(right.relation) || left.from.localeCompare(right.from) || left.to.localeCompare(right.to));
    return { nodes, edges };
};
export const findPatternNode = (graph, id) => {
...
export const findRelatedPatterns = (graph, id) => {
    ...
    return related.sort((left, right) => left.relation.localeCompare(right.relation) || left.pattern.knowledgeId.localeCompare(right.pattern.knowledgeId));
};
export const summarizePatternLayers = (graph) => {
    const summarize = (values) => Object.entries(values.reduce((acc, value) => {
